...
/**
 * Function to update the service metrics based on token usage and costs.
 * @param serviceId - The ID of the LLM service.
 * @param costs - The calculated costs for the chat generation.
 * @param inputTokens - The number of input tokens.
 * @param outputTokens - The number of output tokens.
 * @param debugCostSource - Debug information about the cost source.
 */
export function metricsStoreAddChatGenerate(
  ...
  console.log(`[Metrics Store] Adding chat metrics for ${llm.id} (${debugCostSource}): input=${inputTokens}, output=${outputTokens}`);
  console.log(`[Metrics Store] Costs:`, JSON.stringify(costs));

  if (!llm.service) {
    console.log(`[Metrics Store] No service defined for LLM ${llm.id} - skipping metrics tracking`);
    return;
  }

  const serviceId = llm.service;
  const timestamp = Date.now();
  console.log(`[Metrics Store] Updating metrics for service: ${serviceId} at ${new Date(timestamp).toISOString()}`);

  serviceMetricsAtom.update(state => {
    // Initialize service metrics if not present
    if (!state[serviceId]) {
      console.log(`[Metrics Store] Initializing metrics for new service: ${serviceId}`);
      state[serviceId] = initialServiceMetrics();
    }

    // Update the metrics for this service
    const updatedMetrics = updateServiceMetrics(
      state[serviceId],
      costs,
      inputTokens,
      outputTokens,
    ...
    console.log(`[Metrics Store] Updated metrics for ${serviceId}:`, {
      totalInputTokens: updatedMetrics.totalInputTokens,
      totalOutputTokens: updatedMetrics.totalOutputTokens,
      totalCosts: updatedMetrics.totalCosts,
      usageCount: updatedMetrics.usageCount
    });

    // Update the atom state
    state[serviceId] = updatedMetrics;
